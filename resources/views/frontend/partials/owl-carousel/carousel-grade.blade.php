        ...
        <div class="{{-- form-row --}} rounded-0 bpx px-2 alert alert-{{ $alerts[$grade] }}">
          <div class="col-12 d-flex justify-content-between align-items-center h6 px-0">
            {{ !empty($books) ? (check_locale('bn') ? $books[0]->grade->title_bn:$books[0]->grade->title) : '' }} <a class="btn btn-sm btn-{{ $alerts[$grade] }}" href="{{ route('book.grade.browse', $books[0]->grade->slug) }}">@lang('backend/default.view_all') <span v-if="false">(<span class="{{-- badge badge-danger badge-pill --}}">{{ n2l(count($books)) }}</span>)</span></a>
          </div>
