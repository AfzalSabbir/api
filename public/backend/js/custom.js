...
//     let title = $(this).text();
//     $('.app-content').load(newUrl + ' .app-content .v-cloak--hidden', function(){
...
$(document).ready(function(){
    ...
    acceptMethod =  function (encrypted_id) {
        swal({
            title: "Are you sure?",
            text: "We will let him know that you are interested and share your phone number.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willAccept) => {
            if (willAccept) {
                window.location = window.location.pathname+'/accept/'+encrypted_id;
            }
        });
    }
    confirmAcceptMethod =  function (encrypted_id, mobile, user_id) {
        swal({
            title: "Are you sure?",
            text: "Contact: "+mobile+". \n Press OK to confirm!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willConfirmAccept) => {
            if (willConfirmAccept) {
                window.location = window.location.pathname+'/'+user_id;
            }
        });
    }
    /**
     * Inactive alert
     * @param encrypted_id to inactive
     */
...
$(document).ready(function() {
  if($('.fa-envelope').hasClass('text-warning')){
    setTimeout(function() {
      $('.fa-envelope').removeClass('fa-envelope').addClass('fa-envelope-open');
    }, 3000);
    setTimeout(function() {
